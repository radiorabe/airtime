<?php
class Application_Form_StreamsSubForm extends Zend_Form_SubForm
{
    private $prefix;
    private $setting;
    ...
    public function startForm()
    {
        ...
        $adminUser->setLabel(_("Admin User"))
                         ->setValue(Application_Model_Streams::getAdminUser($prefix))
                         ->setValidators(array(
                            array('regex', false, array('/^[^ &<>]+$/', 'messages' => _('Invalid character entered')))))
                         ->setDecorators(array('ViewHelper'));
        if ($disable_all) {
        ...
        $adminPass->setLabel(_("Admin Password"))
                         ->setValue(Application_Model_Streams::getAdminPass($prefix))
                         ->setValidators(array(
                            array('regex', false, array('/^[^ &<>]+$/', 'messages' => _('Invalid character entered')))))
                         ->setDecorators(array('ViewHelper'));
        if ($disable_all) {
        ...
        $this->setDecorators(array(
            array('ViewScript', array('viewScript' => 'form/streams-form.phtml', "stream_number"=>$stream_number, "enabled"=>$enable->getValue(), "liquidsoap_error_msg"=>$liquidsopa_error_msg))
        ));
    }
