...
function redrawAddShowForm($el, form) {
    ...
    $el.find("#calendar-show-style input").each(function(i, el){
        var $input = $(this), 
...
function setAddShowEvents(form) {
    ...
    if(!form.find("#add_show_rebroadcast").attr('checked')) {
        form.find("#calendar-record-rebroadcast > fieldset:not(:first-child)").hide();
    }
    ...
    form.find("#add_show_repeats").click(function(){
        $(this).blur();
        form.find("#calendar-show-when > fieldset:last").toggle();
    ...
    form.find("#add_show_timezone").change(function(){
        ...
        $.post(baseUrl+"calendar/localize-start-end-time",
               {format: "json",
    ...
    form.find("#calendar-show-style .input_text").ColorPicker({
    ...
    form.find(".add-show-submit").click(function(event) {
        ...
        var start_date = $("#add_show_start_date").val(),
            ...
            action = baseUrl+"calendar/"+String(addShowButton.attr("data-action"));
    ...
    function calculateDuration(startDateTime, endDateTime, timezone){
        ...
        loadingIcon.show();
        $.post(
            baseUrl+"calendar/calculate-duration",
...
function showErrorSections() {
    ...
    if($("#calendar-show-what .errors").length > 0) {
        $("#schedule-show-what").show();
    }
    if($("#calendar-show-when .errors").length > 0) {
        $("#schedule-show-when").show();
    }
    if($("#calendar-show-who .errors").length > 0) {
        $("#schedule-show-who").show();
    }
    if($("#calendar-show-style .errors").length > 0) {
        $("#schedule-show-style").show();
    }
    if($("#add_show_rebroadcast_absolute .errors").length > 0) {
        $("#calendar-record-rebroadcast").show();
