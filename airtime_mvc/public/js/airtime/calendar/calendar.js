...
function confirmCancelShow(show_instance_id){
    if (confirm($.i18n._('Cancel Current Show?'))) {
        var url = baseUrl+"calendar/cancel-current-show";
        $.ajax({
            url: url,
...
function uploadToSoundCloud(show_instance_id, el){
    ...
    var url = baseUrl+"calendar/upload-to-sound-cloud",
    	$el = $(el),
...
/**
 * Use user preferences for time scale; defaults to month if preferences was never set
 */
...
/**
 * Use user preferences for time interval; defaults to 30m if preferences was never set
 */
...
$(document).ready(function() {
    ...
    $.contextMenu({
        ...
        build: function($el, e) {
            ...
            $.ajax({
              url: baseUrl+"calendar/make-context-menu",
              type: "GET",
